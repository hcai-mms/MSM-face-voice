...
def read_data(test_file_face=None, test_file_voice=None):
    # used
    test_file_face = './features_ECAPA_FaceNet/veriflist_test_random_unseenunheard_face.csv'
    test_file_voice = './features_ECAPA_FaceNet/veriflist_test_random_unseenunheard_audio.csv'

    # not used
    #test_file_face = './features_basic/face_veriflist_test_random_unseenunheard.csv'
    #test_file_voice = './features_basic/voice_veriflist_test_random_unseenunheard.csv'
    ...
    face_test = face_test[:, :512]
    voice_test = np.asarray(voice_test)
    #voice_test = voice_test[:, :512]
    voice_test = voice_test[:, :192]

    # face_test = torch.from_numpy(face_test).float()
    # voice_test = torch.from_numpy(voice_test).float()

    face_test = face_test.astype(np.float32)
    voice_test = voice_test.astype(np.float32)

    face_test = torch.from_numpy(face_test)
    voice_test = torch.from_numpy(voice_test)
