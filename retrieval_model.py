...
class LinearWeightedAvg(nn.Module):
    def __init__(self, face_feat_dim, voice_feat_dim):
        ...
        # The model learns how much it should trust face features and how much it should trust voice features
...
class FOP(nn.Module):
    def __init__(self, args, face_feat_dim, voice_feat_dim):
        ...
        msm_matrix = np.load('msms/prototypes901.npy')
        msm_matrix = torch.tensor(msm_matrix, dtype=torch.float32).T
        self.register_buffer("maxsep", msm_matrix)
    ...
    def forward(self, faces, voices):
        ...
        # Both shaped torch.Size([batch_size, 900])

        # feats = faces + voices
        # OR
        # linear fusion (which is almost the same)
        feats, faces, voices = self.fusion_layer(faces, voices)

        # All 3 shaped torch.Size([batch_size, 900]) after fusion

        # 1 option: using logits layer
        #logits = self.logits_layer(feats)

        # 2 option: using MSM
        logits = torch.matmul(feats, self.maxsep)

        # (512,900)x(900,901)=(512,901)
        # Logits torch.Size([512, 900])
